...
public class StudentLoginServlet extends HttpServlet
{
    public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
    {
        ...
        try{
            ...
            rs = st.executeQuery("select * from student where rollno = '"+s1+"' and password ='"+s2+"'");
            if(rs.next())
            {
             res.sendRedirect("WelcomeServlet?k="+s1);  
            }
                  // username is passed through s1 to WelcomeServlet by url rewriting method
